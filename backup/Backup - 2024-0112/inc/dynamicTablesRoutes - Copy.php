 ...
 */
    ...
    // $wpdbColumnsResults += [ "columns" => $resultsColumns ];
...
function update_table_data($request)
{
    ...
    error_log('   ... ' . json_encode($request[ 'id' ]));
    error_log('   ... ' . json_encode($request[ 'table_id' ]));
    error_log('   ... ' . json_encode($request[ 'post_id' ]));
    error_log('   ... ' . json_encode($request[ 'table_name' ]));
    // error_log('   ... ' . json_encode($request[ 0 ][ 'columns' ]));
    error_log('   ... ' . json_encode($request[ 'cells' ]));
    ...
    if (isset($request[ 'post_id' ])) {
        $postId = sanitize_text_field($request[ 'post_id' ]);
    } else {
        $postId = null;
    }

    if (isset($request[ 'table_name' ])) {
        $tableName = sanitize_text_field($request[ 'table_name' ]);
    } else {
        $tableName = null;
    }
