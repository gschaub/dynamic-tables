...
class DynamicTablesVersionManagement
{
    ...
    public static function activate_dynamic_tables()
    {
        ...
        if (!isset($current_db_version)) {
            ...
            // Plugin tables
            $dtHeaderTbl = $wpdb->prefix . 'dt_tables_test';
            $dtColumnsTbl = $wpdb->prefix . 'dt_table_columns_test';
            $dtRowsTbl = $wpdb->prefix . 'dt_table_rows_test';
            $dtCellsTbl = $wpdb->prefix . 'dt_table_cells_test';

            /**
             * Create plugin tables
             */
            $sql = "CREATE TABLE $dtHeaderTbl (
    ...
    public static function uninstall_dynamic_tables()
    {
        ...
        $sql = "DROP TABLE IF EXISTS $dtHeaderTbl";
        $wpdb->query($sql);

        $sql = "DROP TABLE  IF EXISTS $dtColumnsTbl";
        $wpdb->query($sql);

        $sql = "DROP TABLE  IF EXISTS $dtRowsTbl";
        $wpdb->query($sql);

        $sql = "DROP TABLE  IF EXISTS $dtCellsTbl";
        $wpdb->query($sql);
