...
class DTBK_Maintenance {
	...
	public function validate_tables() {
		...
		$post_indexed_fields = array( 'post_id', 'parent_post_id', 'table_id', 'block_table_ref', 'resolved' );
		...
		$post_history_indexed_fields = array( 'post_id', 'table_id', 'block_table_ref', 'resolved' );
	...
	private function get_dynamic_table_blocks( $search_post_types = array(), $search_post_statuses = array() ) {
		...
		$allowed_types = get_post_types( array(), 'names' );
	...
	private function is_api_return_error( $response ) {
		...
		if ( $response->is_error() ) {
			...
			if ( is_wp_error( $error ) ) {
				...
				if ( $wp_error_code !== 0 && ( $wp_error_code < 200 || $wp_error_code >= 299 ) ) {
					return true;
				}
			}
		}
	...
	public function perform_indexed_and_search( $searchable_data, $index, $criteria ) {
		...
		foreach ( $criteria as $field => $value ) {
			$lists[] = $index[ $field ][ $value ] ?? array();
			if ( empty( end( $lists ) ) ) {
				return array(); // no matches → short-circuit
			}
		}
	...
	public function perform_indexed_or_search( $searchable_data, $index, $criteria ) {
		$matching_keys = array();
		foreach ( $criteria as $field => $value ) {
			$matching_keys = array_merge( $matching_keys, $index[ $field ][ $value ] ?? array() );
		}
