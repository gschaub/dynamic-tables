...
1. In your WordPress dashboard, go to **Plugins -> Add New**, search for _Dynamic Tables_, click **Install Now** and then **Activate**.
...
4. Start building your table: add columns and rows, set header options, adjust widths, edit content.
   ...
   > **Important:** Deactivating and deleting the plugin does _not_ auto-remove stored tables. However, there is a setting to support the removal of stored tables. Re-activating restores existing tables in posts.
...
- Fix bug that has banded row text color match background color
- Extended REST API to support filtered retrieval of multiple tables
- Create table block through attachment of existing unattached table
